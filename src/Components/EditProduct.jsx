...
export default function EditProduct({ onClose, productId, setMessage, setRefresh }) {
    ...
    useEffect(() => {
        const fetchProductInfos = async () => {
            ...
            try {
                const res = await fetch(`${config.API_BASE}/management/products?${params.toString()}`, {
                    method: 'GET',
                    headers: {
                ...
                setDescp(data[0]?.description)
                setTitle(data[0].name)
                setUnit(data[0].unitPrice)
                setQuantity(data[0].stockQuantity)
                setOldCategories(data[0].categories.map(c => ({ categName: c, status: "chosen" })))
                setDetailsArray(data[0].details)
                setPropertiesArray(data[0].properties)
                setMediaIds(data[0].images?.map(img => img.mediaId))
                setProductImages(data[0].images)
            } catch (error) {
    ...
    useEffect(() => {
        fetch(config.API_BASE + "/management/products/categories", {
            ...
            .then(data => {
                setCategories(data.map(c => ({ categName: c, status: "unchosen" })))
    ...
    useEffect(() => {
        setNewCategories(() => {
            ...
            oldCategories.forEach(item => mergedCategories.set(item.categName, item));

            categories.forEach(item => {
                if (!mergedCategories.has(item.categName)) {
                    mergedCategories.set(item.categName, item);
                }
            });
    ...
    const onEditProduct = () => {
        ...
        if (descp) {
            changes.description = descp
        }
        if (unit) {
            changes.unitPrice = parseInt(unit)
        }
        if (quantity) {
            changes.stock = parseInt(quantity)
        }
        // if (images.length !== 0) {
        //     changes.images = images
        // }
        if (detailsArray.length !== 0) {
            changes.details = detailsArray
        }
        // if (newCategories.length !== 0) {
        //     changes.categories = newCategories.filter(categ => categ.status === "chosen")
        //         .map(categ => categ.categName)
        // }
        if (propertiesArray.length !== 0) {
            changes.properties = propertiesArray
        }
        ...
        fetch(config.API_BASE + "/management/products/edit", {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    setMessage("Faild editing product")
                    setTimeout(() => {
                        setMessage("")
                    }, 3000);
                    setRefresh(p => !p)
    ...
    useEffect(() => {
        if (deletedImageIds.length !== 0) {
            setProductImages(prev => {
                const updatedProductImages = prev
                    .filter(id => !deletedImageIds.includes(id.mediaId))
                return updatedProductImages
            });
        }
    }, [deletedImageIds, refreshImages]);
    ...
    useEffect(() => {
        setImages(() => {
            return productImages.filter(p => !deletedImageIds.includes(p.mediaId))
        })
    }, [productImages, deletedImageIds])

    const makeMediaMain = (index) => {
        fetch(config.API_BASE + "/management/products/media/main", {
            method: "POST",
            credentials: "include",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({ productId : productId,  mediaId: productImages[index].mediaId })
        }).then(res => res.json())
        .then(data=>{
            console.log(data)
            setRefreshImages(p=>!p)
    ...
    return (
        <div className="edit-product">
            ...
            <div className="footer">
                ...
                <button className="edit" onClick={onEditProduct}>Edit product</button>
            </div>
        </div>
    )
}
